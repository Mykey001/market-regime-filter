...
void OnTick()
{
   ...
   if(buySig && !HasOpenPosition(POSITION_TYPE_BUY))
   {
      ...
      if(EnableRegimeFilter && RF_IsRegimeFilterConnected())
      {
          ...
          {
              Print("ML Regime Filter BLOCKED buy trade. Regime: ", RF_GetCurrentRegime(), 
                    ", Confidence: ", DoubleToString(RF_GetRegimeConfidence(), 1), "%");
              return;  // Trade blocked by regime filter
          }
          Print("ML Regime Filter ALLOWED buy trade. Regime: ", RF_GetCurrentRegime(), 
                ", Confidence: ", DoubleToString(RF_GetRegimeConfidence(), 1), "%");
      }
      // ===================================
   ...
   if(sellSig && !HasOpenPosition(POSITION_TYPE_SELL))
   {
      ...
      if(EnableRegimeFilter && RF_IsRegimeFilterConnected())
      {
          ...
          {
              Print("ML Regime Filter BLOCKED sell trade. Regime: ", RF_GetCurrentRegime(), 
                    ", Confidence: ", DoubleToString(RF_GetRegimeConfidence(), 1), "%");
              return;  // Trade blocked by regime filter
          }
          Print("ML Regime Filter ALLOWED sell trade. Regime: ", RF_GetCurrentRegime(), 
                ", Confidence: ", DoubleToString(RF_GetRegimeConfidence(), 1), "%");
      }
      // ===================================
